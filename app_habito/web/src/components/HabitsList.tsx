...
interface HabitsListProps {
    date: Date;
    onConmpletedChanged: (completed: number) => void;
}
...
export function HabitsList({date, onConmpletedChanged} : HabitsListProps){
    ...
    async function handleToggleHabit(habitId: string){
        const isHabitAlreadCompleted = habitsInfo!.completedHabits.includes(habitId);

        await api.patch(`/habits/${habitId}/toggle`);

        let completedHabits: string [] = []

        if(isHabitAlreadCompleted) {
        ...
        onConmpletedChanged(completedHabits.length);
    }
